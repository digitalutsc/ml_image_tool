...
src = 'C:\\D\\DSU\\Dragomans\\testing_radon'
# if the images only needs 90 degrees CCW to become upright, then use cv2.ROTATE_90_COUNTERCLOCKWISE below
# if the images only needs 90 degrees CW to become upright, then use cv2.ROTATE_90_CLOCKWISE below
needed_rotation = cv2.ROTATE_90_COUNTERCLOCKWISE 
...
for filename in os.listdir(src):
    ...
    if filename.endswith(('.jpg', '.jpeg', '.JPEG', '.JPG', '.png', '.gif', '.tif')):
        i += 1
        print(f'The {i}th image: {filename}')
        
        start_time = time.time()
        if not orientation_detect(imgpath, 15, 65, 1, 'max'):
            image = cv2.imread(imgpath)
            rotated = cv2.rotate(image, needed_rotation)
            cv2.imwrite(imgpath, rotated)  # overwrite original
        end_time = time.time()
        single_time = end_time - start_time
        tot_time += single_time
        avg_time = tot_time / i
        print(f"Current image took: {single_time:.2f} seconds")
        print(f'Average processing: {avg_time:.2f} seconds')
        print(f'Total process time: {avg_time * length:.2f} seconds')
        print(f'Time remaining    : {avg_time * length - tot_time:.2f} seconds')
