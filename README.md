...
|---|--------|--------------------------------------|
| 0+1 | `1_gather_images.py` | Flattens the source tree into numbered folders (`0`, `1`, …) of at most **(2³²−1)/2 files** each (NTFS limit ÷ 2; override with `--max-per-folder`), renames images to `<folder>@@!!!!!!@@<index><ext>`, and logs every old→new location into `mapping.csv` (consumed by step 5). Blocks until you confirm you have a backup. |
