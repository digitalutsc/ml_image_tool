...
#
# Please note, since all images are supposed to be vertical, if the function returns false, then the image
# is perceived by the program as a horizontal image. This is clearly a mistake by the program. Therefore, 
# these problem images are copied to mistake_folder so that you can have a look at what parameters should
# be changed to improve the performance.
...
for filename in os.listdir(src):
    ...
    print(f'perceived vertical images percentage: {s/(s+f)}')
end_time = time.time()
